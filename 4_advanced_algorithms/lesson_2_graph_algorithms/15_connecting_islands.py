...
Find bridges in such a way that all islands are connected with minimum
cost of travelling.
...
In this example if we are connecting bridges like this...
    ...
    * between 4 and 3 with cost = 2
    ...then we connect all 4 islands with cost = 6 which is the
    minimum traveling cost.

Hint
In addition to using a graph, you may want to use a custom priority queue for
solving this problem.

If you do not want to create a custom priority queue, you can use Python's
heapq implementation.

Using the heapq module, you can convert an existing list of items into a heap.
...
print("\nAfter pushing, heap looks like: {}".format(heap))
...
print("\nSmallest element in the original list was: {}".format(smallest))
...
# Note: If you insert a tuple inside the heap, the element at 0th index
# of the tuple is used for comparision
...
print("\nAfter pushing, now heap looks like: {}".format(heap))
...
Write code in the following function to find and return the minimum cost
required to travel all islands via bridges.
'''
...
def minimum_cost(graph):
    ...
    # Starting with vertex 1 (any vertex can be chosen)
    start_vertex = 1
