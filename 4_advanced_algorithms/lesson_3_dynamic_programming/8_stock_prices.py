...
prices = [3, 4, 7, 8, 6]
    ...
    Note: This is a shortened array, just for the sake of example — a full set
    of prices for the day would have 13 elements (one price for each 30 minute
    interval betwen 9:30 and 4:00), as seen in the test cases further down in
    this notebook.
...
Fill out the function below and run it against the test cases.
Take into consideration the time complexity of your solution.
'''
...
def max_returns(arr):
    ...
    At each step we will make the greedy choice by choosing prices
    such that our profit is maximum 
    '''
    min_price_index = 0
    max_price_index = 1
