...
(that is, 6 = 2 + 2 + 2), but this would use more coins — and the
problem specifies we should use the smallest number of coins possible.

There's test code below that you can use to check your solution.
And at the bottom of the notebook, you'll find two different possible
solutions.
'''

# Solution One
'''
Let's assume F(Amount) is the minimum number of coins needed to make
a change from coins [C0, C1, C2...Cn-1]
Then, we know that F(Amount) = min(F(Amount-C0), F(Amount-C1),
F(Amount-C2)...F(Amount-Cn-1)) + 1
...
def coin_change(coins, amount):
    ...
    def return_change(remaining):
        ...
        if remaining not in memo:
            # If not previously calculated, calculate it by calling
    ...
    # Return -1 when no change is found.
