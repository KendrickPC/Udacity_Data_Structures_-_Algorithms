# Merge Sort
'''
MergeSort is a divide and conquer algorithm that divides a list
into equal halves until it has two single elements and then
merges the sub-lists until the entire list has been reassembled
in order.
...
Conquer:
Once you have split the list down to single elements, your
mergesort will start merging lists, in order, until you have
reassembled the entire list in order.

Design:
First, let's sketch this out. It's clear we want a recursive
function, but how will it govern its recursion?
'''
...
'''
We have decomposed the problem, now we will address each piece
separately.

Divide:
We can use python's // operator to find a midpoint. If items's
length is even, we will have the midpoint. If items's length is
odd, we will have one half larger by one.
'''
...
'''
With our midpoints, we can slice the lists using python's special
notation. Note, you can improve effencncy by not using the slice
function. Instead, keep track of indicies.
'''
...
The merge function needs to take two sorted lists, and
interleave them, yielding a merged sorted list. We can
accomplish that by tracking an index into both lists, and
once one is exhausted, appending the remaining items from
the other list.
'''
...
def merge(left, right):
    ...
    # Append any leftovers. We've broken from our while loop
    # so we know at least one is empty. The remaining are:
...
print("-------" * 8)
print("\nNow we can combine our TODO resolutions into " +
      "our sketch from above.")
