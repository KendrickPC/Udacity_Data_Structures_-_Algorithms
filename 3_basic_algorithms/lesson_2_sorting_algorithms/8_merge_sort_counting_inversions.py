...
'''
The number of inversions in a disordered list is the number of
pairs of elements that are inverted (out of order) in the list.
...
The number of inversions can also be thought
of in the following manner.

    Given an array arr[0 ... n-1] of n distinct positive
    integers, for indices i and j, if i < j and
    arr[i] > arr[j] then the pair (i, j) is called an
    inversion of arr.

Problem Statement:
Write a function, count_inversions, that takes an array
(or Python list) as input, and returns a count of the
total number of inversions present in the input.
...
def merge_two_sorted_halves(arr, start_one, end_one, start_two, end_two):
    ...
    while index < output_length:
        ...
        if arr[left_index] <= arr[right_index]:
            ...
            # left > right hence an inversion occurs
